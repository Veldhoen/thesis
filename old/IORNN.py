...
class Leaf(Node):
  def __init__(self, cat, index, word=''):
    ...
    self.index = index
    self.word = word
    self.score = Node([Node([self], 'u', 'tanh')], 'score', 'identity')

  def scoreThis(self, theta, wordIndex=-1):
...
#    some = M[0].dot(self.outerA) + M[1].dot(theta['word'][wordIndex]) + b
    ...
    score = activate(np.transpose(theta['scoreM']).dot(u),'tanh')[0]
#    print self, score
    return score
...
#   def backpropOuter(self, theta, index):
#     gradients = np.zeros_like(theta)
#     wscore = self.score(theta)
#     cscore = self.score(theta,index)
#     if wscore<cscore:
#       delta = 1
#       self.score.backpropInner(delta,theta,gradients)
#      self.parent.backpropOuter
#      self.sibling.backpropInner
...
def numericalGradient(theta, nw, w):
  ...
  # create an iterator to iterate over the array, no matter its shape
      ...
      while not it.finished:
        ...
        errorPlus = max(0,1-score0+nw.scoreThis(theta, w))
        theta[name][i] = old - epsilon
        errorMin = max(0,1-score0+nw.scoreThis(theta, w))
